...
class Test_unsettlementList:
    ...
    def test_unsettlementList(self, excel_data, sport_params):
        ...
        if excel_data[12] == '是':
            ...
            if params_list[5] == True:
                if params_list[0] == None:
                    request_body = {"page": 1, "limit": 200, "accountStatus": "", "searchAccountName": ""}
                else:
                    if params_list[1] == None:
                        request_body = {"page": 1, "limit": 200, "accountStatus": params_list[0], "searchAccountName": ""}
                    else:
                        request_body = {"page": 1, "limit": 200, "accountStatus": params_list[0], "searchAccountName": params_list[1]}
                title = params_list[2]
                ...
                get_token = CreditBackGround(mysql_info, mongo_info).get_user_token(request_method='get',request_url=request_url,request_body=request_body)
                ...
                # 执行接口的请求
                response_data = self.session.get(url=request_url, headers=head, params=request_body).json()
                ...
                if not SQLResult_list:
                    ...
                    expectResult = [list(item) for item in SQLResult_list]
