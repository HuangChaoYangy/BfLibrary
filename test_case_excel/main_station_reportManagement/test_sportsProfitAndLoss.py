...
# @Author  : liyang
# @FileName: test_sportsProfitAndLoss.py
# @Software: PyCharm
...
class Test_sportsProfitAndLoss:
    ...
    # 读取excle 里面的用例
    de = DoExcel(file_name=main_station_report_path, sheet_name="sportsProfitAndLoss")
    case_list1 = de.get_case(de.get_sheet())
    de = DoExcel(file_name=main_station_report_path, sheet_name='sports_params')
    ...
    @allure.story('总台-报表管理-球类盈亏-列表详情')
    def test_sportsProfitAndLoss(self, excel_data, sport_params):
        '''
        管理后台-报表管理-球类盈亏-列表详情,为了数据准确就查询头一天的
        ...
        if excel_data[12] == '是':
            ...
            with allure.step(f'查询SQL:{sql_str}'):
                Bf_log('sportsProfitAndLoss').info(f'执行sql:{sql_str}')
                ...
                if actualResult != [] or expectResult != []:
                    for index1, item1 in enumerate(actualResult):
                        for index2, item2 in enumerate(expectResult):
                            if item1[0] == item2[0]:     # 判断球类名称是否相等,若相等,则校验该条数据
                                new_item1 = []
                                new_item2 = []
                    ...
                    with allure.step(f'实际结果：{actualResult}, 期望结果：{expectResult},==》测试通过'):
                        Bf_log('sportsProfitAndLoss').info(f'实际结果:{actualResult}, 期望结果：{expectResult},==》测试通过')
                        DoExcel(main_station_report_path, "sportsProfitAndLoss").write_result(row=int(excel_data[0] + 1),actual_result=f'{actualResult}',
    ...
    # 读取excle 里面的用例
    de = DoExcel(file_name=main_station_report_path, sheet_name="sports_total")
    case_list1 = de.get_case(de.get_sheet())
    de = DoExcel(file_name=main_station_report_path, sheet_name='sports_params_t')
    case_list2 = de.get_case(de.get_sheet())
    @pytest.mark.parametrize('excel_data', case_list1)
    @pytest.mark.parametrize('sport_params', case_list2)
    @pytest.mark.skip(reason='调试代码,暂不执行')
    @allure.story('总台-报表管理-球类盈亏-总计')
    def test_Daily_total(self, excel_data, sport_params):
        '''
        管理后台-报表管理-球类盈亏-总计,默认以"结算时间"查询近7天数据,因定时任务每10分钟跑一次，为了数据准确就查询头一天的
        :param excel_data:  excel中的测试用例
        :param sport_params: excel中的参数化数据
        :return:
        '''
        ...
        if excel_data[12] == '是':
            ...
            with allure.step(f'查询SQL:{sql_str}'):
                Bf_log('sports_total').info(f'执行sql:{sql_str}')
            if not SQLResult_list:
                expectResult = []
            else:
                expectResult = SQLResult_list
            ...
            if len(actualResult) == len(expectResult):
                if actualResult != [] or expectResult != []:
                    ...
                    with allure.step(f'实际结果：{actualResult}, 期望结果：{expectResult},==》测试通过'):
                        Bf_log('sports_total').info(f'实际结果:{actualResult}, 期望结果：{expectResult},==》测试通过')
                        DoExcel(main_station_report_path, "sports_total").write_result(row=int(excel_data[0] + 1),actual_result=f'{actualResult}',
                                                                                   expect_result=f'{expectResult}',is_pass=f"测试通过 \n{ctime}")
            else:
                raise AssertionError(f"接口查询的结果与数据库查询长度不一致!接口为{len(actualResult)},sql为{len(expectResult)}")
    ...
    # 读取excle 里面的用例
    de = DoExcel(file_name=main_station_report_path, sheet_name="sports_detail")
    case_list1 = de.get_case(de.get_sheet())
    de = DoExcel(file_name=main_station_report_path, sheet_name='sports_params_d')
    case_list2 = de.get_case(de.get_sheet())
    @pytest.mark.parametrize('excel_data', case_list1)
    @pytest.mark.parametrize('sport_params', case_list2)
    # @pytest.mark.skip(reason='调试代码,暂不执行')
    @allure.story('总台-报表管理-球类盈亏-详情')
    def test_Daily_detail(self, excel_data, sport_params):
        '''
        管理后台-报表管理-球类盈亏-详情,默认以"结算时间"查询近7天数据,因定时任务每10分钟跑一次，为了数据准确就查询头一天的
        :param excel_data:  excel中的测试用例
        :param sport_params: excel中的参数化数据
        :return:
        '''
        ...
        if excel_data[12] == '是':
            ...
            sport_id = params_list[6]
            sql_str = eval(excel_data[7])

            SQLResult_list = list(MysqlFunc(mysql_info, mongo_info).query_data(sql_str, db_name='bfty_credit'))
            with allure.step(f'查询SQL:{sql_str}'):
                Bf_log('sports_detail').info(f'执行sql:{sql_str}')
            expectResult = []
            if not SQLResult_list:
                expectResult = []
            else:
            ...
            if len(actualResult) == len(expectResult):
                if actualResult != [] or expectResult != []:
                    ...
                    with allure.step(f'实际结果：{actualResult}, 期望结果：{expectResult},==》测试通过'):
                        Bf_log('sports_detail').info(f'实际结果:{actualResult}, 期望结果：{expectResult},==》测试通过')
                        DoExcel(main_station_report_path, "sports_detail").write_result(row=int(excel_data[0] + 1),actual_result=f'{actualResult}',
                                                                                   expect_result=f'{expectResult}',is_pass=f"测试通过 \n{ctime}")
            else:
                raise AssertionError(f"接口查询的结果与数据库查询长度不一致!接口为{len(actualResult)},sql为{len(expectResult)}")
