...
class Test_matchReport:

    # 读取excle 里面的用例
    de = DoExcel(file_name=owner_backer_path, sheet_name="matchReport")
    case_list1 = de.get_case(de.get_sheet())
    de = DoExcel(file_name=owner_backer_path, sheet_name='match_params')
    ...
    @allure.story('总台-代理报表-赛事盈亏')
    def test_matchReport(self, excel_data, sport_params):
        '''
        管理后台-代理报表-赛事盈亏-列表详情,默认以"结算时间"查询近7天数据,因定时任务每10分钟跑一次，为了数据准确就查询头一天的
            ...
            if params_list[6] == True:
                ...
                if response_data['message'] == 'OK':
                    ...
                    for item in APIResult_list:
                        actualResult.append([item['matchId'],item['allAmount'],item['allEfficient'],item['allBackwater'],item['memberWinLose'],item['memberBackwater'],item['memberFinal'],
                ...
                if not SQLResult_list:
                    ...
                    if SQLResult_list == [('串关', None, None, None, None, None, None, None, None, None, None, None, None,
                                           None, None, None, None, None, None, None, None, None)]:
                        expectResult = []
                    else:
                        for item in SQLResult_list:
                            if item == ('串关', None, None, None, None, None, None, None, None, None, None, None, None, None, None, None,
                                ...
                                if item1[0] == item2[0]:     # 判断比赛ID是否相等,若相等,则校验该条数据
                                    ...
                                    if new_item1 == new_item2:
                                        with allure.step(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试通过'):
                                            ...
                                            DoExcel(owner_backer_path, "matchReport").write_result(row=int(excel_data[0]+1),
    ...
    # 读取excle 里面的用例
    de = DoExcel(file_name=owner_backer_path, sheet_name="matchMarket")
    case_list1 = de.get_case(de.get_sheet())
    de = DoExcel(file_name=owner_backer_path, sheet_name='match_params_m')
    case_list2 = de.get_case(de.get_sheet())
    @pytest.mark.parametrize('excel_data', case_list1)
    @pytest.mark.parametrize('sport_params', case_list2)
    @pytest.mark.skip(reason='调试代码,暂不执行')
    @allure.story('总台-代理报表-赛事盈亏-盘口详情')
    def test_matchReportMarket(self, excel_data, sport_params):
        '''
        管理后台-代理报表-赛事盈亏-盘口详情,默认以"结算时间"查询近7天数据,因定时任务每10分钟跑一次，为了数据准确就查询头一天的
        :param excel_data:  excel中的测试用例
        :param sport_params: excel中的参数化数据
        :return:
        '''
        ...
        if excel_data[12] == '是':
            ...
            match_id_list = MysqlQuery(mysql_info, mongo_info).get_matchId_by_sportId_matchReport(sport_id=params_list[2],time=(params_list[0],
                                                                                                   params_list[1]),queryDateType=params_list[3])

            for match_id in match_id_list:
                request_body['matchId'] = match_id
                total_title = f"根据体育类型：{sport_category_id[params_list[2]]}, 按照时间类型：{dateType_dic[params_list[3]]}"
                allure.dynamic.title(total_title)
                title = f"根据体育类型：{sport_category_id[params_list[2]]}, 按照时间类型：{dateType_dic[params_list[3]]}, 查询比赛ID：{match_id}"
                with allure.step(f"执行测试用例:{title}"):
                    Bf_log('matchMarket').info(f"----------------开始执行:{title}------------------------")

                # 获取接口地址和请求方法
                request_url = CreditBackGround(mysql_info, mongo_info).mde_url + excel_data[4]
                with allure.step(f"请求地址： {request_url}"):
                    Bf_log('matchMarket').info(f'请求地址为：{request_url}')
                request_method = excel_data[5]
                ...
                SQLResult_list = list(MysqlFunc(mysql_info, mongo_info).query_data(sql_str, db_name='bfty_credit'))
                with allure.step(f'查询SQL:{sql_str}'):
                    Bf_log('matchMarket').info(f'执行sql:{sql_str}')
                ...
                if len(actualResult) == len(expectResult):
                    if actualResult != [] or expectResult != []:
                        for index1, item1 in enumerate(actualResult):
                            for index2, item2 in enumerate(expectResult):
                                if item1[0] == item2[0]:     # 判断盘口ID是否相等,若相等,则校验该条数据
                                    new_item1 = []
                                    new_item2 = []
                                    for aip_data in item1[1:]:
                                        if aip_data == None or aip_data == 0:
                                    ...
                                    if new_item1 == new_item2:
                                        with allure.step(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试通过'):
                                            Bf_log('matchMarket').info(f'实际结果:{new_item1}, 期望结果：{new_item2},==》测试通过')
                                            DoExcel(owner_backer_path, "matchMarket").write_result(row=int(excel_data[0]+1),
                                                                                                   actual_result=f'{new_item1}',expect_result=f'{new_item2}',
                                                                                                   is_pass=f"测试通过 \n{ctime}")
                                    else:
                                        with allure.step(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试不通过'):
                                            Bf_log('matchMarket').error(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试不通过')
                                            DoExcel(owner_backer_path, "matchMarket").write_result(row=int(excel_data[0]+1),
                                                                                                   actual_result=f'{new_item1}',expect_result=f'{new_item2}',
                                                                                                   is_pass=f"测试不通过 \n{ctime}")
                                    assert new_item1 == new_item2
                        ...
                        with allure.step(f'实际结果：{actualResult}, 期望结果：{expectResult},==》测试通过'):
                            Bf_log('matchMarket').info(f'实际结果:{actualResult}, 期望结果：{expectResult},==》测试通过')
                            DoExcel(owner_backer_path, "matchMarket").write_result(row=int(excel_data[0] + 1),actual_result=f'{actualResult}',
                                                                                       expect_result=f'{expectResult}',is_pass=f"测试通过 \n{ctime}")
    ...
    # 读取excle 里面的用例
    de = DoExcel(file_name=owner_backer_path, sheet_name="matchOrder_d")
    case_list1 = de.get_case(de.get_sheet())
    de = DoExcel(file_name=owner_backer_path, sheet_name='match_params_d')
    case_list2 = de.get_case(de.get_sheet())
    @pytest.mark.parametrize('excel_data', case_list1)
    @pytest.mark.parametrize('market_params', case_list2)
    # @pytest.mark.skip(reason='调试代码,暂不执行')
    @allure.story('总台-代理报表-赛事盈亏-根据盘口查看注单详情')
    def test_matchReportMarketOrderDetail(self, excel_data, market_params):
        '''
        ...
        if excel_data[12] == '是':
            ...
            if params_list[8] == True:
                ...
                # 根据球类获取所有的比赛id,区分单注和串关,单注则直接是比赛id,串关则为串关
                match_id_list = MysqlQuery(mysql_info, mongo_info).get_match_id_by_sportId_matchReport(sport_id=params_list[2],time=(params_list[0], params_list[1]),
                                                                                                       queryDateType=params_list[3])
                testCase_title = f"根据体育类型：'{sport_category_id[params_list[2]]}', 日期类型：'{dateType_dic[request_body['dateType']]}' 查询赛事盈亏"
                allure.dynamic.title(testCase_title)

                for match_id in match_id_list:          # 遍历比赛列表
                    # match_id = "串关"
                    # match_id = match_id_list[0]
                    request_body['matchId'] = match_id
                    market_id_list = MysqlQuery(mysql_info, mongo_info).get_market_id_by_matchId_matchReport(match_id=match_id,time=(params_list[0], params_list[1]),
                                                                                                             queryDateType=params_list[3])

                    if request_body['matchId'] != "串关":
                        for market in market_id_list:            # 遍历盘口列表
                            request_body['marketId'] = market
                            title = f"根据体育类型：'{sport_category_id[params_list[2]]}', 比赛ID'{match_id}', 盘口ID'{market}' 查看注单详情, 查询日期：'{request_body['begin']} -- {request_body['end']}'"
                            with allure.step(f"执行测试用例:{title}"):
                                Bf_log('matchOrder_d').info(f"----------------开始执行:{title}------------------------")

                            # 获取接口地址
                            request_url = CreditBackGround(mysql_info, mongo_info).mde_url + excel_data[4]
                            with allure.step(f"请求地址： {request_url}"):
                                Bf_log('matchOrder_d').info(f'请求地址为：{request_url}')
                            ...
                            SQLResult_list = list(MysqlFunc(mysql_info, mongo_info).query_data(sql_str, db_name='bfty_credit'))
                            with allure.step(f'查询SQL:{sql_str}'):
                                Bf_log('matchOrder_d').info(f'执行sql:{sql_str}')
                            ...
                            if len(actual_result) == len(expect_result):
                                if actual_result != [] or expect_result != []:
                                    for index1, item1 in enumerate(actual_result):
                                        for index2, item2 in enumerate(expect_result):
                                            if item1[2] == item2[2]:  # 判断注单号是否相等,若相等,则校验该条数据
                                                ...
                                                if new_item1 == new_item2:
                                                    with allure.step(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试通过'):
                                                        Bf_log('matchOrder_d').info(f'实际结果:{new_item1}, 期望结果：{new_item2},==》测试通过')
                                                        DoExcel(owner_backer_path, "matchOrder_d").write_result(row=int(excel_data[0]+1),
                                                                                                               actual_result=f'{new_item1}',expect_result=f'{new_item2}',
                                                                                                               is_pass=f"测试通过 \n{ctime}")
                                                else:
                                                    with allure.step(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试不通过'):
                                                        Bf_log('matchOrder_d').error(f'实际结果：{new_item1}, 期望结果：{new_item2},==》测试不通过')
                                                        DoExcel(owner_backer_path, "matchOrder_d").write_result(row=int(excel_data[0]+1),
                                                                                                               actual_result=f'{new_item1}',expect_result=f'{new_item2}',
                                                                                                               is_pass=f"测试不通过 \n{ctime}")
                                                    # continue
                                    ...
                                    with allure.step(f'实际结果：{actual_result}, 期望结果：{expect_result},==》测试通过'):
                                        Bf_log('matchOrder_d').info(f'实际结果:{actual_result}, 期望结果：{expect_result},==》测试通过')
                                        DoExcel(owner_backer_path, "matchOrder_d").write_result(row=int(excel_data[0] + 1),actual_result=f'{actual_result}',
                    ...
                    elif request_body['matchId'] == "串关":
                        request_body['marketId'] = ""
                        request_body['tournamentId'] = "串关"

                        title = f"根据体育类型：'{sport_category_id[params_list[2]]}', 类型：'{match_id}' 查看注单详情, 查询时间：'{request_body['begin']} -- {request_body['end']}'"
                        with allure.step(f"执行测试用例:{title}"):
                            Bf_log('matchOrder_d').info(f"----------------开始执行:{title}------------------------")
...
if __name__ == "__main__":
    ...
    pytest.main(["test_matchReport.py",'-vs', '-q', '--alluredir', '../report/tmp',])  # '--clean-alluredir'
