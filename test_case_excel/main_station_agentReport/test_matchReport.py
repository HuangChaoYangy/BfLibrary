...
class Test_matchReport:
    ...
    def test_matchReport(self, excel_data, sport_params):
        ...
        if excel_data[12] == '是':
            ...
            request_body['end'] = CommonFunc().get_current_time_for_client(time_type="ctime", day_diff=params_list[1])
            request_body['queryDateType'] = params_list[2]
            request_body['matchId'] = params_list[5]
            title = params_list[3]
            allure.dynamic.title(title)
            with allure.step(f"执行测试用例:{title}"):
    ...
    def test_matchReportMarket(self, excel_data, market_params):
        ...
        if excel_data[12] == '是':
            ...
            if params_list[8] == True:
                ...
                match_id_list = MysqlQuery(mysql_info, mongo_info).get_match_id_by_sportId_matchReport(sport_id=params_list[2],
