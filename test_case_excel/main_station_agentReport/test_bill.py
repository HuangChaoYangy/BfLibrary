...
class Test_bill:
    ...
    def test_billOrderDetail(self, excel_data, sport_params):
        ...
        if excel_data[12] == '是':
            ...
            params_list = sport_params
            request_body = eval(excel_data[6])             # 将字符串转成相应的对象（如list、tuple、dict和string之间的转换）
            request_body['date'] = CommonFunc().get_current_time_for_client(time_type="ctime", day_diff=params_list[0])
            title = params_list[1]
            allure.dynamic.title(title)
            with allure.step(f"执行测试用例:{title}"):
                ...
                for item in APIResult_list:
                    for detail in item['options']:
                        ...
                        actualResult.append([item['account'],item['name'],item['orderNo'],item['betTime'],item['sportType'],item['betType'],
                                             ...
                                             item['settlementTimeStr'],item['betResult'], item['betIp'] + ' / ' + item['betIpAddress'],item['betAmount'], item['winOrLose'], item['validAmount'],
            ...
            # 执行SQL,SQL写法f{"参数"}
            begin = CommonFunc().get_current_time_for_client(time_type="ctime", day_diff=params_list[2])
            end = CommonFunc().get_current_time_for_client(time_type="ctime", day_diff=params_list[3])
            sql_str = eval(excel_data[7])
