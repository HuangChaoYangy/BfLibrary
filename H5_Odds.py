...
class H5_Bfclient_odds(object):
    ...
    def get_h5_odds_match_list(self, sport_name, token, event_type="INPLAY", sort=1, odds_type=1):
        '''
        获取滚球、今日、早盘、串关赛事列表
        :param sport_name:
        :param token:
    ...
    def check_odds(self, match_id, token, odds_type, sport_name):
        ...
        :return:
        '''
        uk_outcomes = self.get_h5_match_all_outcomes(match_id = match_id, token=token, odds_Type=1, sport_name=sport_name)    # 获取欧赔赔率投注项列表
        # print(uk_outcomes)
        odds_type_value = odds_type_dic[odds_type]
        # print(odds_type_value)
        outcomes_exact = self.get_h5_match_all_outcomes(match_id = match_id, token=token, odds_Type=odds_type_value, sport_name=sport_name)   # 获取实际客户端的赔率投注项列表
            ...
            for item in uk_outcomes:
                ...
                if outcome_id_uk == outcome_id_exact:
                    ...
                    #         print('盘口ID：' + marketid +' 切换盘口类型之后，【赔率会变】------------------------测试通过------------------------')
                        ...
                        print("盘口下注项ID：%s ,【---赔率类型---】欧赔: %.2f,港赔: %.2f, 实际印尼赔: %.3f " % (outcome_id_uk, outcome_odds_uk, outcome_odds_gang, outcome_odds_exact))
...
if __name__ == "__main__":
    ...
    token_list = ['0b696766ef0b4f19bbfe984868009e5e']
    odds_type_dic = {"港赔": '2', "马来赔": '3', "印尼赔": '4'}

    # 验证遍历所有比赛,赔率是否正确
    for sport_name in ["足球", "篮球", "网球", "排球", "羽毛球", "乒乓球", "棒球", "冰上曲棍球"]:
        print("----------------------------------------------------------------------------------      " + sport_name + "      ----------------------------------------------------------------------------------------          ")
        match_list = bf.get_h5_odds_match_list(sport_name=sport_name, token=token_list[0], event_type="TODAY", sort=1)[0]

        for match_id in match_list:                       # 遍历所有match_list列表，检查赔率是否一致
            if match_id not in ["sr:match:27267978"]:           # 此数据有问题,判断比赛ID为有问题的话,跳过该比赛
                bf.check_odds(match_id=match_id, token=token_list[0], odds_type='印尼赔', sport_name=sport_name)

    # 验证单场比赛,赔率是否正确
    # bf.check_odds(match_id ="sr:match:26877044", token=token_list[0], odds_type='马来赔', sport_name="足球")
