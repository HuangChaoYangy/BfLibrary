...
#     cm = CommonFunc()
#
#     @pytest.mark.parametrize('inBody, expData', ya.get_yaml_data('../data/AgentCommissionPayoff.yaml'))
#
#     def test_AgentComissionPatoff(self, inBody, expData):
#         '''
#         总台-业主管理
#         :param inBody:
#         :param expData:
#         :return:
#         '''
#         apiRes = IncorrectBackend(mysql_info,mongo_info).getPayoffcommission(inData=inBody)
#         sqlRes = MysqlQuery(mysql_info, mongo_info).get_userAgent_commission(expData=expData)
#
#         self.cm.check_live_bet_report_new(apiRes, sqlRes)
