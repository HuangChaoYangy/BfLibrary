...
# @Author  : liyang
# @FileName: test_UserManagement.py.py
# @Software: PyCharm
...
class Test_Report01(object):
    ...
    @pytest.mark.parametrize('inBody, expData', ya.get_yaml_data('../data/BackendUserManagement.yaml'))
    ...
    def test_BackendOwnerManagement(self, inBody, expData):
        ...
        :return:
        '''
        apiRes = IncorrectBackend(mysql_info,mongo_info).getBackendUserManagement(inData=inBody)
        sqlRes = MysqlQuery(mysql_info, mongo_info).getBackendUserManagement_sql(expData=expData)
        # print(apiRes)
        # print(sqlRes)
