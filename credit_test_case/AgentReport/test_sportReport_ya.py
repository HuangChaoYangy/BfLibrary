# -*- coding: utf-8 -*-
# @Time    : 2022/7/22 14:33
# @Author  : liyang
# @FileName: test_sportReport_ya.py
# @Software: PyCharm
...
@allure.feature('总台-代理报表-球类报表')
class Test_sportReport_ya:
    ...
    def test_sportReport(self, inBody, expData, url=url_data):
        ...
        sql = MysqlQuery(mysql_info, mongo_info).credit_sportReport_query(expData=expData,queryType='market')[1]
        with allure.step(f'查询SQL:{sql}'):
            Bf_log('sportReport').info(f'执行sql:{sql}')
        ...
        for marketId in market_id_list:  # 遍历盘口列表
            if marketId != "串关":
                ...
                sql_str = f"SELECT CONCAT(d.account,'/',d.login_account) as '账号/登入账号',d.`name` '名称',a.order_no as '注单号',a.create_time as '投注时间',(CASE WHEN a.sport_category_id= 1 " \
                          ...
                          f"CONCAT( home_team_name, ' Vs ', away_team_name ) '赛事名称',IF(is_live=3,'早盘','滚球盘') '赛事类型',market_name  '盘口名称',specifier '亚盘口',outcome_name '投注项名称'," \
                          ...
                          f"{date_type} BETWEEN '{begin}' and  '{end}' and b.market_id='{marketId}' AND a.bet_type=1 {sport_Str} ORDER BY a.create_time DESC"
...
if __name__ == "__main__":
    ...
    pytest.main(["test_sportReport_ya.py", '-vs', '-q', '--alluredir', '../report/tmp','--clean-alluredir'])  # '--clean-alluredir', '-n=4'
