...
class Test_multitermReport:
    ...
    def test_multitermReportDetail(self, inBody, expData, request=request_data):
        '''
        管理后台-代理报表-混合过关-注单详情,默认以"结算时间"查询近7天数据,因定时任务每10分钟跑一次，为了数据准确就查询头一天的
        :param excel_data:  excel中的测试用例
        :param sport_params: excel中的参数化数据
        :return:
        '''
        ...
        for account in account_id_list:  # 遍历会员账号列表
            ...
            sql_str = f"SELECT CONCAT(d.account,'/',d.login_account) as '账号/登入账号',d.`name` '名称',a.order_no as '注单号',a.create_time as '投注时间',(CASE WHEN a.sport_category_id= 1 " \
                      ...
                      f"{date_type} BETWEEN '{begin}' and  '{end}' and d.account='{account}' AND a.bet_type>1 {sport_Str} ORDER BY a.create_time DESC"
