# -*- coding: utf-8 -*-
# @Time    : 2022/7/20 18:46
# @Author  : liyang
# @FileName: test_winLoseSimple_ya.py
# @Software: PyCharm
...
@allure.feature('总台-报表管理-总代盈亏(简易)')
class Test_winLoseSimple_yaml(object):
    ...
    # YamlFileData().get_testcase_params(csv_path=csv_url_winlose_simple_d, yaml_file=winlose_simple_url_d, new_yaml_file=winlose_simple_url_new_d)
    # yaml_data = Yaml_data().read_yaml_file(yaml_file=winlose_simple_url_new_d, isAll=False)
    # url_data = Yaml_data().read_yaml_file(yaml_file=winlose_simple_url_new_d, isAll=True)[0]['request']
    # # @pytest.mark.skip(reason="调试代码,暂不执行")
    # @pytest.mark.parametrize('inBody, expData', yaml_data)
    # @allure.story('报表管理-总代盈亏(简易)-注单详情')
    # def test_winLoseSimpleDetail(self, inBody, expData, url=url_data):
    #     '''
    #     管理后台-报表管理-总代盈亏(简易)-注单详情
    #     :param inBody:
    #     :param expData:
    #     :return:
    #     '''
    ...
    yaml_data = Yaml_data().read_yaml_file(yaml_file=simpleUrl, isAll=False)
    url_data = Yaml_data().read_yaml_file(yaml_file=simpleUrl, isAll=True)[0]['request']
    # @pytest.mark.skip(reason="调试代码,暂不执行")
    @pytest.mark.parametrize('inBody, expData', yaml_data)
    @allure.story('报表管理-总代盈亏(简易)-注单详情')
    def test_winLoseSimpleDetail(self, inBody, expData, request=url_data):
        '''
        管理后台-报表管理-总代盈亏(简易)-注单详情
        :param inBody:
        :param expData:
        ...
        for user_id in user_id_list:
            total_title = f"根据会员账号查询总代盈亏(简易)-注单详情,  查询日期范围：'{ctime} -- {etime}'"
            ...
            with allure.step(f"执行测试用例:{title}"):
                Bf_log('winLoseSimple').info(f"----------------开始执行:{title}------------------------")

            with allure.step(f"请求地址 {request_url}"):
                Bf_log('winLoseSimple').info(f'请求地址为:{request_url}')
                ...
                for item in APIResult_list:
                    for detail in item['options']:
                        ...
                        actualResult.append([item['account'], item['name'], item['orderNo'], item['betTimeStr'], item['sportType'],bet_type,
                             ...
                             item['betResult'], item['betIp'] + ' / ' + betIpAddress, item['odds'], item['betAmount'],item['memberWinOrLose'],item['validAmount'],
                             ...
                             item['level3Percentage'], item['level3WinOrLose'],item['level3CommissionRatio'], item['level3Commission'],
            ...
            with allure.step(f'查询SQL:{sql_str}'):
                Bf_log('winLoseSimple').info(f'执行sql:{sql_str}')
            ...
            if len(actual_result) == len(expectResult):
                if actual_result != [] or expectResult != []:
                    ...
                    with allure.step(f'实际结果：{actual_result}, 期望结果：{expectResult},==》测试通过'):
                        Bf_log('winLoseSimple').info(f'实际结果:{actual_result}, 期望结果：{expectResult},==》测试通过')
...
if __name__ == "__main__":
    ...
    pytest.main(["test_winLoseSimple_ya.py",'-vs', '-q', '--alluredir', '../report/tmp','--clean-alluredir'])  # '--clean-alluredir'
