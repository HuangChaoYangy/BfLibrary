...
class Test_clientReport_yaml(object):
    ...
    YamlFileData().get_testcase_params(csv_path=csv_url_client_d, yaml_file=client_report_url_d, new_yaml_file=client_report_url_new_d)
    yaml_data = Yaml_data().read_yaml_file(yaml_file=client_report_url_new_d, isAll=False)
    url_data = Yaml_data().read_yaml_file(yaml_file=client_report_url_new_d, isAll=True)[0]['request']
    # @pytest.mark.skip(reason="调试代码,暂不执行")
    @pytest.mark.parametrize('inBody, expData', yaml_data)
    @allure.story('客户端盈亏-根据客户端查看详情')
    def test_clientReportDetail(self, inBody, expData, url=url_data):
        '''
        管理后台-客户端盈亏-根据客户端查看详情
        :param inBody:
        :param expData:
        :return:
        '''
...
if __name__ == "__main__":
    ...
    pytest.main(["test_clientReport_ya.py",'-vs', '-q', '--alluredir', '../report/tmp','-n=auto','--clean-alluredir'])  # '--clean-alluredir'
